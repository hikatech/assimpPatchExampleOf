...
ofShader ofMaterialFix::shaderNoTexture;
ofShader ofMaterialFix::shaderTexture2D;
ofShader ofMaterialFix::shaderTextureRect;
bool ofMaterialFix::shadersInitialized = false;
size_t ofMaterialFix::shaderLights = 0;
...
ofMaterialFix::ofMaterialFix()
{
}

void ofMaterialFix::begin() const
{
	if (ofGetGLRenderer()) {
...
void ofMaterialFix::initShaders(ofGLProgrammableRenderer & renderer) const
{
...
const ofShader & ofMaterialFix::getShader(int textureTarget, ofGLProgrammableRenderer & renderer) const
{
...
static const string fragmentShaderFixed = R"(
    // FragShader in ofMaterialFix
...
#define SPECULAR_REFLECTION
	...
	vec2 repeated(in SAMPLER tex, in vec2 texCoord){
		return vec2(mod(texCoord.x,textureSize(tex,0).x),mod(texCoord.y,textureSize(tex,0).y));
	}
	...
	void main (void){
		...
		#ifdef HAS_TEXTURE
			vec4 tex = TEXTURE(tex0, repeated(tex0, outtexcoord));
...
static string fragmentSource(string defaultHeader, int maxLights, bool hasTexture) {
	return shaderHeader(defaultHeader, maxLights, hasTexture) + fragmentShaderFixed;
}