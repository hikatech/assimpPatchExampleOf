...
unsigned int ofxAssimpModelLoaderFix::initImportProperties(bool optimize) {
...
//-------------------------------------------
void ofxAssimpModelLoaderFix::createEmptyModel() {
	if (scene) {
		clear();
		scene.reset();
	}
...
void ofxAssimpModelLoaderFix::setNormalizationFactor(float factor) {
	normalizeFactor = factor;
}

//-------------------------------------------
void ofxAssimpModelLoaderFix::calculateDimensions() {
	if (!scene) return;
	ofLogVerbose("ofxAssimpModelLoaderFix") << "calculateDimensions(): inited scene with "
	...
	if (abs(normalizedScale) < std::numeric_limits<float>::epsilon()) {
		ofLogWarning("ofxAssimpModelLoaderFix") << "Error calculating normalized scale of scene" << endl;
		normalizedScale = 1.0;
	}
	else {
...
//-------------------------------------------
void ofxAssimpModelLoaderFix::createLightsFromAiModel() {
	lights.clear();
...
void ofxAssimpModelLoaderFix::optimizeScene() {
...
//-------------------------------------------
void ofxAssimpModelLoaderFix::clear() {

	ofLogVerbose("ofxAssimpModelLoaderFix") << "clear(): deleting GL resources";
...
//------------------------------------------- update.
void ofxAssimpModelLoaderFix::update() {
	if (!scene) return;
	updateAnimations();
...
void ofxAssimpModelLoaderFix::updateAnimations() {
...
void ofxAssimpModelLoaderFix::updateBones() {
	if (!hasAnimations()) {
		return;
	}
...
void ofxAssimpModelLoaderFix::updateGLResources() {
...
void ofxAssimpModelLoaderFix::updateModelMatrix() {
...
//------------------------------------------- animations.
bool ofxAssimpModelLoaderFix::hasAnimations() {
	return animations.size() > 0;
}

unsigned int ofxAssimpModelLoaderFix::getAnimationCount() {
	return animations.size();
}

ofxAssimpAnimation & ofxAssimpModelLoaderFix::getAnimation(int animationIndex) {
	animationIndex = ofClamp(animationIndex, 0, animations.size() - 1);
	return animations[animationIndex];
}

void ofxAssimpModelLoaderFix::playAllAnimations() {
	for (unsigned int i = 0; i<animations.size(); i++) {
		animations[i].play();
	}
}

void ofxAssimpModelLoaderFix::stopAllAnimations() {
	for (unsigned int i = 0; i<animations.size(); i++) {
		animations[i].stop();
	}
}

void ofxAssimpModelLoaderFix::resetAllAnimations() {
	for (unsigned int i = 0; i<animations.size(); i++) {
		animations[i].reset();
	}
}

void ofxAssimpModelLoaderFix::setPausedForAllAnimations(bool pause) {
	for (unsigned int i = 0; i<animations.size(); i++) {
		animations[i].setPaused(pause);
	}
}

void ofxAssimpModelLoaderFix::setLoopStateForAllAnimations(ofLoopType state) {
	for (unsigned int i = 0; i<animations.size(); i++) {
		animations[i].setLoopState(state);
	}
}

void ofxAssimpModelLoaderFix::setPositionForAllAnimations(float position) {
	for (unsigned int i = 0; i<animations.size(); i++) {
...
// DEPRECATED.
void ofxAssimpModelLoaderFix::setAnimation(int animationIndex) {
	if (!hasAnimations()) {
		return;
	}
...
// DEPRECATED.
void ofxAssimpModelLoaderFix::setNormalizedTime(float time) {
	if (!hasAnimations()) {
		return;
	}
...
// DEPRECATED.
void ofxAssimpModelLoaderFix::setTime(float time) {
	if (!hasAnimations()) {
		return;
	}
	setAnimation(currentAnimation); // call this again to clamp animation index, in case the model is reloaded.
...
// DEPRECATED.
float ofxAssimpModelLoaderFix::getDuration(int animationIndex) {
	if (!hasAnimations()) {
		return 0;
	}
...
//------------------------------------------- meshes.
bool ofxAssimpModelLoaderFix::hasMeshes() {
	return modelMeshes.size() > 0;
}

unsigned int ofxAssimpModelLoaderFix::getMeshCount() {
...
//-------------------------------------------
void ofxAssimpModelLoaderFix::getBoundingBoxWithMinVector(aiVector3D* min, aiVector3D* max)
{
	aiMatrix4x4 trafo;
...
//-------------------------------------------
void ofxAssimpModelLoaderFix::setPosition(float x, float y, float z) {
	pos.x = x;
	pos.y = y;
	pos.z = z;
...
//-------------------------------------------
void ofxAssimpModelLoaderFix::setScale(float x, float y, float z) {
	scale.x = x;
	scale.y = y;
	scale.z = z;
...
//-------------------------------------------
void ofxAssimpModelLoaderFix::setScaleNormalization(bool normalize) {
...
//-------------------------------------------
void ofxAssimpModelLoaderFix::setRotation(int which, float angle, float rot_x, float rot_y, float rot_z) {
...
//--------------------------------------------------------------
void ofxAssimpModelLoaderFix::drawWireframe() {
	draw(OF_MESH_WIREFRAME);
}

//--------------------------------------------------------------
void ofxAssimpModelLoaderFix::drawFaces() {
	draw(OF_MESH_FILL);
}

//--------------------------------------------------------------
void ofxAssimpModelLoaderFix::drawVertices() {
	draw(OF_MESH_POINTS);
}
...
//-------------------------------------------
vector<string> ofxAssimpModelLoaderFix::getMeshNames() {
...
//-------------------------------------------
int ofxAssimpModelLoaderFix::getNumMeshes() {
	return scene->mNumMeshes;
}

//-------------------------------------------
ofMesh ofxAssimpModelLoaderFix::getMesh(string name) {
	ofMesh ofm;
	...
	if (!aim) {
		ofLogError("ofxAssimpModelLoaderFix") << "getMesh(): couldn't find mesh: \"" << name << "\"";
		return ofm;
	}
...
//-------------------------------------------
ofMesh ofxAssimpModelLoaderFix::getMesh(int num) {
	ofMesh ofm;
	if ((int)scene->mNumMeshes <= num) {
		ofLogError("ofxAssimpModelLoaderFix") << "getMesh(): mesh id " << num
...
//-------------------------------------------
ofMesh ofxAssimpModelLoaderFix::getCurrentAnimatedMesh(string name) {
	...
	ofLogError("ofxAssimpModelLoaderFix") << "getCurrentAnimatedMesh(): couldn't find mesh: \"" + name << "\"";
	return ofMesh();

}

//-------------------------------------------
ofMesh ofxAssimpModelLoaderFix::getCurrentAnimatedMesh(int num) {
	if ((int)modelMeshes.size() <= num) {
		ofLogError("ofxAssimpModelLoaderFix") << "getCurrentAnimatedMesh(): mesh id: " << num
			<< "out of range for total num meshes: " << scene->mNumMeshes;
		return ofMesh();
	}
...
//-------------------------------------------
ofTexture ofxAssimpModelLoaderFix::getTextureForMesh(string name) {
	for (int i = 0; i<(int)modelMeshes.size(); i++) {
		if (string(modelMeshes[i].mesh->mName.data) == name) {
	...
	ofLogError("ofxAssimpModelLoaderFix") << "getTextureForMesh(): couldn't find mesh: \"" + name << "\"";
	return ofTexture();
}

//-------------------------------------------
ofTexture ofxAssimpModelLoaderFix::getTextureForMesh(int i) {
	if (i < modelMeshes.size()) {
		if (modelMeshes[i].hasTexture()) {
			return modelMeshes[i].getTextureRef();
		}
	}
	ofLogError("ofxAssimpModelLoaderFix") << "getTextureForMesh(): mesh id: " << i
		<< "out of range for total num meshes: " << scene->mNumMeshes;
	return ofTexture();
}

//-------------------------------------------
ofPoint ofxAssimpModelLoaderFix::getPosition() {
	return pos;
}

//-------------------------------------------
ofPoint ofxAssimpModelLoaderFix::getSceneCenter() {
	return aiVecToOfVec(scene_center);
}

//-------------------------------------------
float ofxAssimpModelLoaderFix::getNormalizedScale() {
	return normalizedScale;
}

//-------------------------------------------
ofPoint ofxAssimpModelLoaderFix::getScale() {
	return scale;
}

//-------------------------------------------
ofMatrix4x4 ofxAssimpModelLoaderFix::getModelMatrix() {
	return modelMatrix;
}

//-------------------------------------------
ofPoint ofxAssimpModelLoaderFix::getSceneMin(bool bScaled) {
...
//-------------------------------------------
ofPoint	ofxAssimpModelLoaderFix::getSceneMax(bool bScaled) {
...
//-------------------------------------------
int ofxAssimpModelLoaderFix::getNumRotations() {
	return rotAngle.size();
}

//-------------------------------------------
ofPoint ofxAssimpModelLoaderFix::getRotationAxis(int which) {
...
//-------------------------------------------
float ofxAssimpModelLoaderFix::getRotationAngle(int which) {
...
//-------------------------------------------
const aiScene* ofxAssimpModelLoaderFix::getAssimpScene() {
	return scene.get();
}

//--------------------------------------------------------------
void ofxAssimpModelLoaderFix::enableTextures() {
	bUsingTextures = true;
}

//--------------------------------------------------------------
void ofxAssimpModelLoaderFix::enableNormals() {
	bUsingNormals = true;
}

//--------------------------------------------------------------
void ofxAssimpModelLoaderFix::enableColors() {
	bUsingColors = true;
}

//--------------------------------------------------------------
void ofxAssimpModelLoaderFix::enableMaterials() {
	bUsingMaterials = true;
}

//--------------------------------------------------------------
void ofxAssimpModelLoaderFix::disableTextures() {
	bUsingTextures = false;
}

//--------------------------------------------------------------
void ofxAssimpModelLoaderFix::disableNormals() {
	bUsingNormals = false;
}

//--------------------------------------------------------------
void ofxAssimpModelLoaderFix::disableColors() {
	bUsingColors = false;
}

//--------------------------------------------------------------
void ofxAssimpModelLoaderFix::disableMaterials() {
